...
(defun play (&key (p1-name "Player 1")
                  (p2-name "Player 2")
                  (p1-type 'human)
                  (p2-type 'human)
                  (p1-symbol 'X)
    ...
    (loop until (game-over-all? board)
          do (case p1-type
               (mm
                ...
                (let ((ai-move (second (minimax-value board t))))
                  (move-ai p1-name 1 board ai-move formats symbol-hash)))
               (em
                (format formats "I'm thinking...~%")
                (let ((ai-move (second (minimax-value board t :other-func #'expectimax-value))))
                  (move-ai p1-name 1 board ai-move formats symbol-hash)))
               (rand
                (format formats "I'm thinking...~%")
                (move-rand p1-name 1 board formats symbol-hash))
               (human
                (move p1-name 1 board formats symbol-hash))
               (otherwise
          ...
          do (case p2-type
               (mm
                (format formats "I'm thinking...~%")
                (let ((ai-move (second (minimax-value board nil))))
                  (move-ai p2-name 2 board ai-move formats symbol-hash)))
               (em
                (format formats "I'm thinking...~%")
                (let ((ai-move (second (minimax-value board nil :other-func #'expectimax-value))))
                  (move-ai p2-name 2 board ai-move formats symbol-hash)))
               (rand
                (format formats "I'm thinking...~%")
                (move-rand p2-name 2 board formats symbol-hash))
               (human
                (move p2-name 2 board formats symbol-hash))
               (otherwise
...
(defun legal-move? (horiz vert board)
  (if (or (not (numberp vert))
          (not (numberp horiz))
          (> horiz (array-dimension board 0))
          (> vert (array-dimension board 1))
          (< horiz 1)
          (< vert 1)
...
(defvar pruned-nodes 0)
(defvar all-nodes 0)
(DEFUN minimax-value (board max? &key (other-func #'minimax-value)
                                      (alpha -2)
                                      (beta 2))
  (if (game-over-all? board)
    ...
    (let ((best-move nil)
      ...
      (dolist (move moves)
        (incf all-nodes)
        (if (if max? (> best-value beta) (< best-value alpha))
            nil
          (progn (play-move board move (if max? 1 2))
                 (let* ((move-info (funcall other-func board (not max?) :other-func #'minimax-value (if max? :alpha :beta) best-value))
...
(DEFUN expectimax-value (board max? &key (other-func #'minimax-value)
                                         (alpha 0)
                                         (beta 0))
  (if (game-over-all? board)
      (if (game-over-mm? board t)
        ...
        (if (game-over-mm? board nil)
          ...
          (list 0 nil 0)))
    (let ((best-move nil)
          (best-value (if max? -99999 99999))
          (moves (get-moves board))
          (sum 0))
      (dolist (move moves)
        (play-move board move (if max? 1 2))
        (let* ((move-info (funcall other-func board (not max?) :other-func #'expectimax-value))
               (move-value (first move-info)))
              (setf sum (+ sum move-value))
              (when (or (and (= move-value best-value) (= (random 2) 0))
                        (funcall (if max? #'> #'<) move-value best-value))
                (setf best-value move-value)
                (setf best-move move))
              (undo-move board move)))
      (list (/ sum (length moves)) best-move))))
...
(defun move-rand (player symbol board formats symbol-hash)
  (let ((horiz (random (array-dimension board 0)))
        (vert (random (array-dimension board 1))))
...
(defun play-move (board move symbol)
  (let ((horiz (first move))
        (vert (second move)))
    (if (= (aref board vert horiz) 0)
        (setf (aref board vert horiz) symbol)
      (break "AHHHH"))))

(defun undo-move (board move)
    (setf (aref board (second move) (first move)) 0))
  
(defun get-moves (board)
  (let ((possible-moves nil))
    (dotimes (horiz (array-dimension board 0))
      (dotimes (vert (array-dimension board 1))
...
(defun lots-smart (num-games &key (p1-type 'mm)
                                  (p2-type 'mm)
                                  (board-size 3))
  (let ((p1-wins 0)
        (p2-wins 0)
    ...
    (loop until (= games-left 0)
        do (let ((winner (play :board-size board-size :p1-type p1-type :p2-type p2-type :formats nil)))
